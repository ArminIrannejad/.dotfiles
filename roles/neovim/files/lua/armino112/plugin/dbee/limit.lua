...
-- gigabytes more, and the host gets oom-killed a handful of queries in.
--
-- the result window only ever renders one page, so nothing is lost by bounding
-- the statement instead. `-- nolimit` in the query opts out.
...
Handler.connection_execute = function(self, id, query)
  local capped = M.apply(query)
  if capped then
    vim.notify(
      ("dbee: capped at %d rows (`-- %s` to opt out)"):format(M.config.rows, M.config.marker),
      vim.log.levels.INFO
    )
    query = capped
  end
  return connection_execute(self, id, query)
end
